...
var _ = BeforeSuite(func() {
	...
	impsReconciler := &ImagePullSecretReconciler{
		...
		ResourceReconciler:        reconciler.NewReconcilerWith(k8sManager.GetClient()),
		Recorder:                  k8sManager.GetEventRecorderFor("imagepullsecrets-controller"),
		PeriodicReconcileInterval: 300 * time.Second,
		Log:                       impsLogger,
	}
	err = impsReconciler.SetupWithManager(k8sManager)
	Expect(err).ToNot(HaveOccurred())
